function getPods(kubeConfiguration, callback){
    ...
    k8sApi.listNamespacedPod('default','true').then((res) => {
        ...
        {
//            console.log(pod);
            clusterlogger.log('Container status : ',pod.status.phase)
            clusterlogger.log('Container status : ',pod.metadata.name)
            pods.push( {
        ...
        clusterlogger.log('request rejected : ', rej.statusCode);
...
function waitForPodToBeOnline(k8Configuration, podName, retryNo, callback)
{
    getPodByName(k8Configuration,podName,(err,pod) => {
        ...
        {
            clusterlogger.log('Waiting for pod to come online ', podName);
            return setTimeout(() => {
...
function waitForAllThePodsToBeOnline(k8Configuration, retryNo, callback)
{
    getPods(k8Configuration,(err,pods) => {
        ...
        if (err || keepWaiting)
        {
            clusterlogger.log('Waiting for pods to come online ');
            return setTimeout(() => {
